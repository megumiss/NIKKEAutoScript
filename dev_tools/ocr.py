...
def rgb_to_hsv(rgb):
    """
    Convert RGB tuple to HSV tuple (OpenCV HSV space).
    """
...
def calc_text_color_tolerance_from_two_rgb(primary_rgb, secondary_rgb):
    """
    以 primary_rgb 为中心，计算覆盖 secondary_rgb 的 HSV 容差。
    返回: (tolerance_h, tolerance_s, tolerance_v)
    """
    h1, s1, v1 = rgb_to_hsv(primary_rgb)
    h2, s2, v2 = rgb_to_hsv(secondary_rgb)

    dh = abs(h1 - h2)
    dh = min(dh, 180 - dh)  # Hue 环绕
    ds = abs(s1 - s2)
    dv = abs(v1 - v2)
    return int(dh), int(ds), int(dv)
...
if __name__ == '__main__':
    h, s, v = calc_text_color_tolerance_from_two_rgb((248, 252, 254), (217, 217, 217))
    ...
    # 初始化OCR实例（只需一次）
    ocr_engine = init_ocr(det_model=CUSTOM_DET_MODEL, rec_model=CUSTOM_REC_MODEL, lang=LANGUAGE)
