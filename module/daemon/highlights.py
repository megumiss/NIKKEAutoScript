...
from module.daemon.assets import HIGHLIGHTS_RED, PLAY_HIGHLIGHTS
...
class Highlights(UI, DaemonBase):
    def run(self):
        ...
        while 1:
            ...
            # 花絮红点
            if click_timer.reached() and self.appear(HIGHLIGHTS_RED, offset=(10, 10), threshold=0.95):
                while 1:
                    self.device.screenshot()

                    # 点击红点，进入鉴赏列表
                    if self.appear_then_click(HIGHLIGHTS_RED, offset=(10, 10), threshold=0.95, interval=1):
                        click_timer.reset()
                        continue
                    # 红点消失，播放花絮
                    if not self.appear(HIGHLIGHTS_RED, offset=(10, 10)):
                        # 寻找一个鉴赏
            ...
            # 没有红点，切换nikke
            if self.appear(DETAIL_CHECK, offset=10, threshold=0.85) and not self.appear(
                HIGHLIGHTS_RED, offset=(10, 10)
            ):
                self.switch_to_next()
        ...
        while 1:
            ...
            if self.appear(PLAY_HIGHLIGHTS, offset=30):
                if not confirm_timer.started():
                    confirm_timer.start()
                if confirm_timer.reached() and self.appear_then_click(PLAY_HIGHLIGHTS, offset=30):
                    logger.info('Highlights end')
                    continue
            else:
                confirm_timer.clear()
