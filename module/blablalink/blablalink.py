...
class Blablalink(UI):
    ...
    def _prepare_config(self):
        ...
        self.common_headers["x-common-params"] = json.dumps(
            common_params, ensure_ascii=False
        )
        # 获取user-agent
        ...
        for attempt in range(max_retries):
            ...
            try:
                response = self.session.request(
                    method, url, headers=self.common_headers, **kwargs
                )
    ...
    def get_post_list(self, exclude_liked: bool = False) -> list:
        ...
        try:
            ...
            if response.get("code") != 0:
                logger.warning(
                    f"Failed to get post list: {response.get('msg', 'Unknown error')}"
                )
                return []
    ...
    def like_post(self, post_uuid: str) -> bool:
        ...
        try:
            ...
            result = self._request_with_retry(
                "POST", url, json={"post_uuid": post_uuid, "type": 1, "like_type": 1}
            )
    ...
    def open_post(self, post_uuid: str) -> bool:
        ...
        try:
            ...
            result = self._request_with_retry(
                "POST", url, json={"post_uuid": post_uuid}
            )
        ...
        try:
            ...
            if response.get("code") == 0:
                ...
                for group in response.get("data", {}).get("list", []):
                    emojis.extend(
                        [icon["pic_url"] for icon in group.get("icon_list", [])]
                    )
                if emojis:
    ...
    def post_comment(self):
        ...
        request_body["content"] = (
            f'<p><img src="{emoji_url}?imgtype=emoji" width="60" height="60"></p>'
        )
        ...
        try:
            ...
            for task in tasks:
                ...
                if "每日簽到" in task_name:
                    ...
                    logger.info(
                        f"Signin task: {'completed' if is_completed else 'pending'}"
                    )

                elif "按讚" in task_name:
                    status["like_completed"] = is_completed
                    logger.info(
                        f"Like task: {'completed' if is_completed else 'pending'}"
                    )

                elif "瀏覽" in task_name:
                    status["browse_completed"] = is_completed
                    logger.info(
                        f"Browse task: {'completed' if is_completed else 'pending'}"
                    )

                elif "評論" in task_name:
                    status["comment_completed"] = is_completed
                    logger.info(
                        f"Comment task: {'completed' if is_completed else 'pending'}"
                    )
    ...
    def cdk(self):
        ...
        logger.info(
            f"CDK redemption completed: {success_count}/{len(unredeemed_cdks)} successful"
        )
    ...
    def get_cdk_redemption_history(self) -> list:
        ...
        try:
            ...
            if result.get("code") != 0:
                logger.error(
                    f"Failed to get CDK history: {result.get('msg', 'Unknown error')}"
                )
                return []
