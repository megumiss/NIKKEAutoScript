...
def _driver_scheme_missing_driver(name):
    ...
    依赖罗技 G HUB 虚拟 HID 驱动，未安装时启动后所有点击/滑动都会失败。
    """
    ...
    if deep_get(config, keys='NKAS.PCClientInfo.ControlScheme', default='pyautogui') != 'driver':
        return False
    from module.tools.logi_driver import probe_device
...
async def start(request: Request):
    ...
    if name != 'all':
        ...
        if _driver_scheme_missing_driver(name):
            logger.warning(f'Instance "{name}" start blocked: control scheme "driver" requires the Logitech virtual HID driver')
            return JSONResponse({
                'status': 'error', 'code': 'driver_not_installed',
                'message': 'Control scheme "driver" requires the Logitech virtual HID driver. '
                           'Install it from Tools > Logi Driver first.',
            }, status_code=400)
    ...
    for instance in names:
        ...
        if _driver_scheme_missing_driver(instance):
            logger.warning(f'Instance "{instance}" start blocked: control scheme "driver" requires the Logitech virtual HID driver')
            results.append({
