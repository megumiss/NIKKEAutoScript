...
class SimulationRoom(UI):
    ...
    def get_next_event(self):
        for i in ENEMY_EVENT_CHECK.match_several(
            self.device.image, offset=5, threshold=0.95, static=False
        )[:3]:
        ...
        if NORMAL_CHECK._button_offset:
            ...
            EnemyEvent(
                button=NORMAL_CHECK.location, config=self.config, device=self.device
            ).run()
        ...
        if self.appear(BOSS_EVENT_CHECK, offset=(30, 30), static=False):
            ...
            EnemyEvent(
                button=BOSS_EVENT_CHECK.location, config=self.config, device=self.device
            ).run()
            return
        ...
        if HARD_CHECK._button_offset:
            ...
            EnemyEvent(
                button=HARD_CHECK.location, config=self.config, device=self.device
            ).run()
    ...
    def choose_effect(self, skip_first_screenshot=True):
        ...
        if not self.appear(
            MAX_EFFECT_COUNT, offset=(10, 10), static=False, threshold=0.96
        ):
            ...
            while 1:
                ...
                if click_timer.reached() and self.appear(
                    SKIP_CHECK, offset=(30, 30), interval=5, static=False
                ):
        ...
        if self.appear(REPEATED_EFFECT_CHECK, offset=(5, 5), static=False):
            ...
            while 1:
                ...
                if click_timer.reached() and self.appear_then_click(
                    CANCEL, offset=(30, 30), interval=5, static=False
                ):
                    confirm_timer.reset()
                    click_timer.reset()
                    continue
                ...
                if (
                    not self.appear(
                        REPEATED_EFFECT_CHECK, offset=(30, 30), static=False
                    )
                    and not self.appear(
                        SELECT_REWARD_EFFECT_CHECK, offset=(30, 30), static=False
                    )
            ...
            while 1:
                ...
                if (
                    not self.appear(
                        MAX_EFFECT_COUNT_CHECK, offset=(30, 30), static=False
                    )
                    and not self.appear(
                        SELECT_REWARD_EFFECT_CHECK, offset=(30, 30), static=False
                    )
    ...
    def _run(self):
        while 1:
            self.get_next_event()
            if self.appear(
                SELECT_REWARD_EFFECT_CHECK, offset=(5, 5), interval=5, static=False
            ):
                self.choose_effect()
    ...
    def run(self):
        try:
            ...
            logger.warning(
                "failed to overcome the current battle, will skip simulation task"
            )
            self.handle_failed()
    ...
    def ensure_into_simulation(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if click_timer.reached() and self.appear_then_click(
                ARK_GOTO_SIMULATION_ROOM, offset=(30, 30), interval=5
            ):
                confirm_timer.reset()
                click_timer.reset()
                continue
            ...
            if self.appear(SIMULATION_CHECK, offset=(30, 30)) or self.appear(
        ...
        while 1:
            ...
            if click_timer.reached() and self.appear_then_click(
                START_SIMULATION, offset=(30, 30), interval=5
            ):
                confirm_timer.reset()
                click_timer.reset()
                continue
            ...
            if click_timer.reached() and self.appear(
                START_SIMULATION_CONFIRM, offset=(30, 30), interval=2
            ):
            ...
            #     click_timer.reset()
            #     continue
            # elif click_timer.reached() and self.appear_then_click(START_SIMULATION_CONFIRM, offset=(5, 5), static=False):
            #     confirm_timer.reset()
            #     click_timer.reset()
            #     continue
            ...
            if self.appear(
                SELECT_REWARD_EFFECT_CHECK, offset=(30, 30), interval=5, static=False
            ):
                break

            if (
                self.appear(SIMULATION_CHECK, offset=(30, 30), threshold=0.86)
                and confirm_timer.reached()
            ):
    ...
    def ensure_into_next_region(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if click_timer.reached() and self.appear_then_click(
                GOTO_NEXT_REGION, offset=(30, 30), interval=2
            ):
                click_timer.reset()
                continue

            if (
                not self.appear(END_SIMULATION, offset=(5, 5))
                and confirm_timer.reached()
            ):
                break
    ...
    def end_simulation(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if click_timer.reached() and self.appear(
                END_SIMULATION_CHECK, offset=(30, 30), static=False
            ):
                self.device.click_minitouch(520, 800)
                logger.info(
                    "Click %s @ %s" % (point2str(520, 800), "END_SIMULATION_CONFIRM")
                )
                click_timer.reset()
                continue

            if click_timer.reached() and self.appear(
                CHOOSE_INITIAL_EFFECT_CHECK, offset=(10, 10), static=False
            ):
                if self.appear_then_click(
                    NOT_CHOOSE_INITIAL_EFFECT, offset=(10, 10), interval=3, static=False
                ):
