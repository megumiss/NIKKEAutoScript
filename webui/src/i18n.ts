...
export const staticLabels: Record<string, Record<string, string>> = {
  ...
  '游戏多开': { 'en-US': 'Game multi-open', 'ja-JP': 'ゲーム多重起動' },
  '复制一份游戏安装目录，重命名新副本的启动器与游戏程序，并写入新副本的路径配置。复制前请关闭游戏和启动器。': { 'en-US': 'Copy the game installation directory, rename the launcher and game executable of the new copy, and write its path configuration. Close the game and launcher before copying.', 'ja-JP': 'ゲームのインストールフォルダをコピーし、新しいコピーのランチャーとゲーム実行ファイルをリネームしてパス設定を書き込みます。コピー前にゲームとランチャーを終了してください。' },
  ...
  '罗技驱动': { 'en-US': 'Logi Driver', 'ja-JP': 'ロジクールドライバー' },
  'driver 控制方案依赖罗技 G HUB 虚拟 HID 驱动。安装会把驱动文件复制到 C:\\ProgramData\\LGHUB 并运行安装器。': { 'en-US': 'The "driver" control scheme requires the Logitech G HUB virtual HID driver. Installation copies the driver files to C:\\ProgramData\\LGHUB and runs the installer.', 'ja-JP': 'driver 制御方式はロジクール G HUB 仮想 HID ドライバーが必要です。インストールはドライバーファイルを C:\\ProgramData\\LGHUB にコピーし、インストーラーを実行します。' },
  '当前系统不支持安装罗技驱动': { 'en-US': 'Logitech driver installation is not supported on this system', 'ja-JP': 'このシステムではロジクールドライバーのインストールに未対応です' },
  ...
  '将把驱动文件复制到 C:\\ProgramData\\LGHUB 并安装罗技虚拟 HID 驱动，确定继续？': { 'en-US': 'This copies the driver files to C:\\ProgramData\\LGHUB and installs the Logitech virtual HID driver. Continue?', 'ja-JP': 'ドライバーファイルを C:\\ProgramData\\LGHUB にコピーし、ロジクール仮想 HID ドライバーをインストールします。続行しますか？' },
  ...
  '将移除系统中的罗技虚拟 HID 驱动，之后 driver 控制方案的实例将无法启动，确定继续？': { 'en-US': 'This removes the Logitech virtual HID driver from the system; instances using the "driver" control scheme will no longer start. Continue?', 'ja-JP': 'システムからロジクール仮想 HID ドライバーを削除します。driver 制御方式のインスタンスは起動できなくなります。続行しますか？' },
  ...
  '罗技驱动未安装': { 'en-US': 'Logi driver not installed', 'ja-JP': 'ロジクールドライバー未インストール' },
  '当前实例的控制方案为 driver，需要先安装罗技 G HUB 虚拟 HID 驱动。请前往「常用工具 → 罗技驱动」完成安装后再启动。': { 'en-US': 'This instance uses the "driver" control scheme, which requires the Logitech G HUB virtual HID driver. Go to "Tools → Logi Driver" to install it before starting.', 'ja-JP': 'このインスタンスは driver 制御方式を使用しており、ロジクール G HUB 仮想 HID ドライバーのインストールが必要です。「ツール → ロジクールドライバー」でインストールしてから起動してください。' },
  ...
  '复制文件': { 'en-US': 'Copying files', 'ja-JP': 'ファイルコピー中' }, '重命名程序': { 'en-US': 'Renaming executables', 'ja-JP': '実行ファイル名変更中' },
