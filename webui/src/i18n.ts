...
export const staticLabels: Record<string, Record<string, string>> = {
  ...
  '不选择星期则保持各任务原设置': { 'en-US': 'Leave days unselected to keep each task\'s current days', 'ja-JP': '曜日を選択しない場合は各タスクの設定を維持' },
  '所选任务将统一设置为该周期；每周/每月任务使用第一个时间点': { 'en-US': 'All selected tasks switch to this cadence; weekly/monthly tasks use the first time point.', 'ja-JP': '選択したタスクはこの周期に統一されます。毎週/毎月タスクは最初の時間帯を使用します。' },
