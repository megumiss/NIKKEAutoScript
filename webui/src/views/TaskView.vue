...
const { schemaReady, collapsed, activeGroup, taskSchema, importBusy, notifyTestBusy, vddBusy } = storeToRefs(workspace)
const { isWideField, save, saveValue, datetimeValue, scheduleDatetimeSave, flushDatetimeSave, clearField, pickedPath, importInterception, testNotify, startTool, refreshMonitors, vddSet } = workspace
...
function onSelectOpen(field: any) { if (field.key.endsWith('.ScreenNumber')) refreshMonitors(field) }
              ...
              <template v-for="field in group.fields" :key="field.key">
                <div :id="`field-${field.key}`" class="field" :class="{ 'field-wide': isWideField(field) }">
                  ...
                  <div class="field-control">
                    ...
                    <AppSelect v-else-if="field.widget === 'select'" :model-value="field.value" :options="field.options" :disabled="field.display !== 'show'" @change="(value: any) => saveValue(field, value).catch(() => {})" @open="onSelectOpen(field)"/>
