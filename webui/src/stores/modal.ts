...
export const useModalStore = defineStore('modal', () => {
  ...
  const modal = ref<{ type: '' | 'create' | 'rename' | 'delete' | 'resetDeploy' | 'confirm' | 'alert' | 'avatar'; name: string; renameTarget: string; origin: string; template: string; avatar: string; avatarTarget: string; keepSchedule: boolean; busy: boolean }>({ type: '', name: '', renameTarget: '', origin: 'template-nkas', template: 'intl', avatar: '', avatarTarget: '', keepSchedule: false, busy: false })
  ...
  function openCreateModal() { modal.value = { type: 'create', name: defaultInstanceName(), renameTarget: '', origin: useInstancesStore().instances[0]?.name || 'template-nkas', template: 'intl', avatar: '', avatarTarget: '', keepSchedule: false, busy: false } }
  function openRenameModal(name: string) { modal.value = { type: 'rename', name, renameTarget: name, origin: '', template: '', avatar: '', avatarTarget: '', keepSchedule: false, busy: false } }
  function openDeleteModal(name: string) { modal.value = { type: 'delete', name, renameTarget: '', origin: '', template: '', avatar: '', avatarTarget: '', keepSchedule: false, busy: false } }
  function openResetDeployModal() { modal.value = { type: 'resetDeploy', name: '', renameTarget: '', origin: '', template: 'intl', avatar: '', avatarTarget: '', keepSchedule: false, busy: false } }
  function openAvatarModal(name: string) {
    const instance = useInstancesStore().instances.find(item => item.name === name)
    modal.value = { type: 'avatar', name, renameTarget: '', origin: '', template: '', avatar: instance?.avatar ?? '', avatarTarget: name, keepSchedule: false, busy: false }
  }
  ...
  function openConfirmModal(message: string, action: () => void | Promise<void>) {
    ...
    modal.value = { type: 'confirm', name: '', renameTarget: '', origin: '', template: '', avatar: '', avatarTarget: '', keepSchedule: false, busy: false }
  }
  ...
  function openAlertModal(title: string, message: string) {
    ...
    modal.value = { type: 'alert', name: '', renameTarget: '', origin: '', template: '', avatar: '', avatarTarget: '', keepSchedule: false, busy: false }
  }
  async function confirmModal() {
    ...
    try {
      if (m.type === 'create') {
        ...
        if (!name) return
        await api.post('/api/instances', { name, origin: m.origin, avatar: m.avatar, keep_schedule: m.keepSchedule })
